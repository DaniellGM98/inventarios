<?php
	...
	class SalidaProduccionModel {
        ...
        public function getByDate($inicio, $fin) {
			$this->response->result = $this->db
				->from($this->table)
				->select(null)->select("$this->table.id_salida, $this->table.destino as cliente, $this->table.fecha, $this->table.peso_total, $this->table.nota_salida, CONCAT_WS(' ', CONCAT_WS(' ', DATE_FORMAT($this->table.fecha, '%d-%m-%Y')), SUBSTRING($this->table.fecha, 12, 20)) as fechaf")
